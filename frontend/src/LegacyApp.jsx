...
/**
 * LegacyApp — the pre-redesign collapsible-section page, moved verbatim from
 * the old `App.jsx` (IMPL-SPEC §10) so nothing is lost while the bench
 * redesign is the default surface. Reachable at `/legacy/*`.
 *
 * This is a move, not a rewrite: the only changes from the original are the
 * route strings, which are prefixed with `/legacy` so this page's internal
 * navigation (the mobile nav bar, the experiment-tree "configure" deep link)
 * stays self-contained under `/legacy` instead of escaping into the new bench
 * routes that now own the top-level `/running`, `/datasets`, `/schedule` etc.
 * paths. `/plots/fullscreen` is no longer handled here — it is now a
 * top-level route in `App.jsx` shared by both surfaces.
 */
...
function LegacyApp() {
  ...
  const getPageFromPath = (pathname) => {
    if (pathname.startsWith("/legacy/datasets")) return "datasets";
    if (pathname.startsWith("/legacy/plots")) return "plots";
    if (pathname.startsWith("/legacy/schedule")) return "schedule";
    if (pathname.startsWith("/legacy/configure")) return "configure";
    if (pathname.startsWith("/legacy/running")) return "running";
    if (pathname.startsWith("/legacy/logs")) return "logs";
  ...
  const handlePageChange = (page) => {
    ...
    const route = routes[page] || "/legacy/schedule";
    navigate(route);
  };
  ...
  const handleSelect = (node) => {
    ...
    navigate({ pathname: "/legacy/configure", search: params.toString() });
