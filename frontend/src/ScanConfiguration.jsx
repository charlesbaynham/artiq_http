...
function ScanConfiguration({ scan, schemata, onChange }) {
    const { axes = [], num_repeats = 1, no_axes_mode = 'single', randomise_order_globally = false, skip_on_persistent_transitory_error = false } = scan;

    // Get scannable parameters (filter by is_scannable flag)
    const scannableParams = React.useMemo(() => {
        if (!schemata) return [];
        return Object.values(schemata).filter(schema =>
            schema.spec && schema.spec.is_scannable
        );
    }, [schemata]);
    ...
    return (
        <Card className="mb-3">
            ...
            <Card.Body>
                ...
                <h6>Scan Axes</h6>
                {axes.length === 0 ? (
                    <p className="text-muted small">No scan axes defined. Click "Add Scan Axis" to create one.</p>
                ) : (
                    axes.map((axis, index) => {
                            ...
                            <Card key={index} className="mb-2">
                                <Card.Body className="py-2 px-3">
                                    <Row className="mb-2">
                                        <Col>
                                            ...
                                            <Form.Select
                                                size="sm"
                                                value={axis.fqn || ''}
                                                onChange={(e) => handleAxisChange(index, 'fqn', e.target.value)}
                                            >
                                                ...
                                                {scannableParams.map(param => (
                                                    <option
                                                        key={param.fqn}
                                                        value={param.fqn}
                                                        disabled={scannedFqns.has(param.fqn) && axis.fqn !== param.fqn}
                                                    >
                                    ...
                                    <Row className="g-2">
                                        ...
                                        <Col>
                                            ...
                                            <Form.Control
                                                ...
                                                onChange={(e) => handleAxisChange(index, 'range.num_points', parseInt(e.target.value))}
                                                min="1"
                                                step="1"
                                            />
                                        </Col>
                                    ...
                                    <Form.Check
                                        ...
                                        onChange={(e) => handleAxisChange(index, 'range.randomise_order', e.target.checked)}
                                    />
                                </Card.Body>
                            </Card>
                        );
                    ...
                    <Col md={6}>
                        ...
                        <InputGroup size="sm">
                            <Form.Control
                                ...
                                onChange={(e) => handleNumRepeatsChange(parseInt(e.target.value) || 1)}
                                min="1"
                                step="1"
                                disabled={isInfinite}
