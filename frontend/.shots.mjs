...
async function shot(name, { width, height, path, waitMs = 3500 } = {}) {
  ...
  page.on("console", (m) => {
    if (m.type() === "error") {
      const t = m.text();
      if (t.includes("ws://localhost:5173") || t.includes("[vite] failed to connect")) return;
      errors.push(t);
    }
  });
  ...
  console.log(`wrote ${outPath}${errors.length ? " CONSOLE ERRORS: " + JSON.stringify(errors) : " (no console errors)"}`);
  await ctx.close();
  return outPath;
}
...
await shot("04-plots-standalone-1440x900", { width: 1440, height: 900, path: "/plots" });
await shot("05-plots-standalone-375x812", { width: 375, height: 812, path: "/plots" });
