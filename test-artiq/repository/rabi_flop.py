...
class Readout(Fragment):
    def build_fragment(self):
        self.setattr_param("num_shots", IntParam, "Number of shots", 100, is_scannable=False)
...
class RabiFlopSim(ExpFragment):
    def build_fragment(self):
        ...
        self.setattr_param("rabi_freq", FloatParam, "Rabi frequency", 1.0 * MHz, unit="MHz", min=0.0)
        self.setattr_param("duration", FloatParam, "Pulse duration", 0.5 * us, unit="us", min=0.0)
        self.setattr_param("detuning", FloatParam, "Detuning", 0.0 * MHz, unit="MHz")
        self.setattr_param("initial_state", EnumParam, "Initial state", InitialState.bright)
